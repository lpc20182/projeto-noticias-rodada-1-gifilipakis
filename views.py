...
def noticias_ano(request, ano):
    try:
        noticia = Noticia.objects.filter(data_de_publicacao__year=ano)
        return render (request, 'app_noticias/noticias_ano.html',
        {
            'ano':ano,
            'noticias':noticia
        })
    except Noticia.DoesNotExist:
        return Http404('Nenhuma notícia foi publicada neste ano')